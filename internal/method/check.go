...
func (m *Method) IsVisible(node *cdp.Node) (bool, error) {
	...
	ctx, cancel := context.WithTimeout(m.pageCtx, 1000*time.Millisecond) // Short timeout for a quick check
	defer cancel()
...
func (m *Method) IsDisabled(node *cdp.Node) (bool, error) {
	...
	// The `ok` parameter in AttributeValue indicates if the attribute was found.
	err := chromedp.Run(m.pageCtx,
		chromedp.AttributeValue(node.NodeID, "disabled", nil, &isDisabled), // ByNodeID is implicit when sel is NodeID
	)
	// If err is not nil, something went wrong with the query.
	// If err is nil, isDisabled will be true if the "disabled" attribute exists, false otherwise.
