...
func (m *Method) ChooseModelByName(name, modelNameSelector string, modelCategoryContainerSelector string) error {
	...
	for i := 0; i < len(modelCategoryContainers); i++ {
		...
		clickCtx, cancelClick := context.WithTimeout(m.pageCtx, 2500*time.Millisecond)
		...
		if err == nil {
			...
			for j := 0; j < len(modelNameElements); j++ {
				...
				textCtx, _ := context.WithTimeout(m.pageCtx, 1000*time.Millisecond) // Shorter timeout for text retrieval
				...
				if strings.Contains(innerText, name) {
					finalClickCtx, cancelFinalClick := context.WithTimeout(m.pageCtx, 2500*time.Millisecond)
