...
func (m *Method) UploadFiles(base64edImages []string) error {
	...
	log.Debugf("Attempting to upload %d files to input element'", len(filePaths))
	err = chromedp.Run(m.page.GetContext(),
		chromedp.ActionFunc(func(ctx context.Context) error {
			err = page.SetInterceptFileChooserDialog(true).Do(ctx)
			if err != nil {
				log.Errorf("set intercept filechooser dialog failed %v", err)
			}
				...
				case *page.EventFileChooserOpened:
					go func(backendNodeID cdp.BackendNodeID) {
						err = chromedp.Run(ctx,
							dom.SetFileInputFiles(filePaths).WithBackendNodeID(backendNodeID),
						)
						if err != nil {
							log.Fatal(err)
						} else {
							// _ = os.RemoveAll(tempDir) // Clean up the temp dir
						}
						// err = page.SetInterceptFileChooserDialog(false).Do(ctx)
					}(ev.(*page.EventFileChooserOpened).BackendNodeID)
					listenCancel()
				}
			})
			return nil
	...
	if err != nil {
		return fmt.Errorf("could not set files for upload on element: %v", err)
	}
